...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 3 {
        eprintln!("用法: {} <服务器地址:端口> <文件路径1> [文件路径2 ...]", args[0]);
        std::process::exit(1);
    }
...
async fn send_file(server_addr: &str, file_path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    stream.write_all(&(filename_bytes.len() as u64).to_be_bytes()).await?;
    ...
    println!("开始发送文件: {} ({} bytes)", filename, filesize);
