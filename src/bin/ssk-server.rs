...
async fn main() -> Result<()> {
  ...
  tracing_subscriber::fmt()
    .with_env_filter(EnvFilter::from_default_env().add_directive("ssk=info".parse().unwrap()))
    .init();
...
async fn handle_client(mut socket: TcpStream, addr: SocketAddr, upload_dir: &str) -> Result<()> {
  ...
  loop {
    ...
    file.write_all(&buffer[..n]).await?; // 写入文件
  }
